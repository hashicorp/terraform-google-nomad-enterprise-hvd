...
> 📖 See the full [Prerequisites Reference](./docs/prereqs.md) for step-by-step `gcloud` commands covering every item below.
...
>📝 Store these TLS files as GCP Secrets Manager secrets to securely manage your certificates. See the [TLS Certificates Guide](./docs/tls-guide.md) for instructions on generating Nomad-specific certificates using the `nomad tls` CLI.
...
>📝 See the [Prerequisites Reference](./docs/prereqs.md#gcp-secret-manager-secrets) for full `gcloud secrets create` commands for each required secret.

## Usage

> 📖 See the [Deployment Guide](./docs/deployment-guide.md) for the complete end-to-end walkthrough, including server deployment, bootstrap monitoring, client deployment, and CLI configuration.
...
1. If `nomad_acl_enabled` is `true` the Nomad cluster will need to be bootstrapped with the command `nomad acl bootstrap`. This will generate a bootstrap token that can be used to login to the CLI or UI. See the [ACL Guide](./docs/acl-guide.md) for policy authoring and token management.

## Documentation

| Guide | Description |
|---|---|
| [Deployment Guide](./docs/deployment-guide.md) | End-to-end walkthrough: servers, clients, bootstrap monitoring, CLI config |
| [Prerequisites](./docs/prereqs.md) | GCP APIs, networking, Secret Manager secrets, and IAM setup |
| [Architecture Overview](./docs/architecture-overview.md) | GCP resource topology, MIG design, and startup script flow |
| [TLS Certificates Guide](./docs/tls-guide.md) | Generate and rotate Nomad-specific TLS certs using the `nomad tls` CLI |
| [ACL Guide](./docs/acl-guide.md) | Bootstrap ACLs, author policies, manage tokens |
| [Configuration Reference](./docs/configuration-reference.md) | All input variables grouped by functional area |
| [Upgrade Guide](./docs/upgrade-guide.md) | Zero-downtime Nomad version upgrades via MIG rolling replace |
...
| <a name="input_nomad_region"></a> [nomad\_region](#input\_nomad\_region) | Specifies the region of the local agent. A region is an abstract grouping of datacenters. Clients are not required to be in the same region as the servers they are joined with, but do need to be in the same datacenter. | `string` | `null` | no |
